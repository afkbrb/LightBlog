    ...
    <form id="blogForm" enctype="multipart/form-data">
        <table cellspacing="20px" width="100%">
            <tr>
                ...
                <td><input type="text" id="title" name="title" style="width: 400px;" value="${blog.title}"/></td>
            </tr>
            <tr>
                <td>文章图片</td>
            ...
            <tr>
                <td>转载地址</td>
                <td><input type="text" id="reprint" name="reprint" style="width: 400px;" value="${blog.reprint}"></td>
            </tr>
            <tr>
                <td>所属类别：</td>
                <td>
                    <select class="easyui-combobox"  style="width: 154px" id="blogType" name="blogType" editable="false"
                            panelHeight="auto">
                        <option value="">请选择博客类别...</option>
                        <c:forEach var="blogType" items="${blogTypeList }">
                        <option value="${blogType.id }" <c:if test="${blog.typeId == blogType.id}">selected="selected"</c:if>>${blogType.typeName}</option>
                        </c:forEach>
                    </select>
                </td>
            </tr>
            <tr>
                ...
                <td>
                    <div id="content-editormd" class="form-group" style="z-index: 100">
                        <textarea id="content-editormd-markdown-doc" style="display: none">${blog.content}</textarea>
                    </div>
                </td>
            </tr>
            <tr>
...
<script type="text/javascript">
    ...
    function submitData() {
        var title = $("#title").val();
        var typeId = $("#blogType").combobox("getValue");
        ...
        if (title == null || title == '') {
            ...
            $("#blogForm").ajaxSubmit({
                type: "post",
                url: "${pageContext.request.contextPath}/admin/blog/update",
                contentType: false,
                processData: false,
